  ...
  if (before?.status && after?.status) {
    return (
      <div className="flex items-center gap-1.5 text-xs mt-1">
        <span className="bg-red-50 border border-red-100 text-red-600 px-1.5 py-0.5 rounded font-medium">
          {before.status}
        </span>
        <svg className="w-3 h-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        <span className="bg-green-50 border border-green-100 text-green-600 px-1.5 py-0.5 rounded font-medium">
          {after.status}
        </span>
      </div>
    );
      ...
      <td className="table-td">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="text-xs font-medium text-gray-800 truncate">{log.actor?.name || 'System'}</p>
            {log.actor?.role && (
              <span className={`text-xs px-1 py-0 rounded font-medium ${ROLE_BADGE[log.actor.role] || 'bg-gray-100 text-gray-500'}`}>
                {log.actor.role}
              </span>
            )}
          </div>
...
function HourlySparkline({ data }) {
  ...
  return (
    <div className="card mb-5">
      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Today's activity (by hour)</p>
      <div className="flex items-end gap-0.5 h-12">
        {hours.map(({ h, count }) => {
          const pct = (count / maxCount) * 100;
          return (
            <div key={h} className="flex-1 flex flex-col items-center gap-0.5" title={`${h}:00 — ${count} events`}>
              <div
...
export default function AuditLogs() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/audit/export?${params}`, {
  ...
  const handlePurge = async () => {
    const days = window.prompt('Purge logs older than how many days? (default: 365)', '365');
    if (!days) return;
    if (!window.confirm(`This will permanently delete audit logs older than ${days} days. Continue?`)) return;
    setPurging(true);
    try {
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Complete history of every action in the system</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={handlePurge} disabled={purging} className="btn-ghost text-xs py-1.5 text-red-400 hover:text-red-600">
      ...
      {stats?.topActors?.length > 0 && (
        <div className="card mb-5">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Most active this month</p>
          <div className="flex flex-wrap gap-2">
            {stats.topActors.slice(0, 8).map((a, i) => (
              <div key={i} className="flex items-center gap-1.5 bg-gray-50 border border-gray-200 rounded-full px-2.5 py-1">
                <span className={`text-xs font-medium ${ROLE_BADGE[a.role] || 'text-gray-600'}`}>{a.name}</span>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-48 max-w-xs">
          ...
          <input className="input pl-9 text-sm" placeholder="Search logs…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>

        {/* Action filter */}
        <select className="input w-48 text-sm" value={action} onChange={(e) => setAction(e.target.value)}>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-sm text-gray-400">
            {hasFilters ? <>No logs match. <button onClick={clearFilters} className="text-blue-600 hover:underline">Clear filters</button></> : 'No audit logs yet.'}
          </div>
        ) : (
