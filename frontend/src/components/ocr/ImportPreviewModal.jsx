    ...
    try {
      const { data } = await api.post('/ocr/import', {
        leads: selectedLeads.map((l) => ({
          ...
          source: l.source || 'Other', notes: l.extra || '',
        })),
      });
      setResult(data);
  ...
  return (
    <>
      ...
      <div className="modal-panel">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {leads.length} found · {duplicateCount} duplicate{duplicateCount !== 1 ? 's' : ''} · {selectedLeads.length} ready
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            className="w-8 h-8 flex items-center justify-center rounded-full
                       hover:bg-gray-100 text-gray-400 disabled:opacity-40 touch-manipulation"
          >
        ...
        {/* Summary strip */}
        <div className="flex items-center gap-4 px-4 sm:px-6 py-2.5 bg-gray-50 border-b border-gray-100 flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-3 scrollbar-thin">
          {checking ? (
            ...
            leads.map((lead) => (
              <LeadPreviewCard key={lead.id} lead={lead} onChange={handleChange} />
            ))
          )}
        </div>
              ...
              >
                {importing ? (
                  ...
                  <>
                    ...
                    Import Selected ({selectedLeads.length})
                  </>
                )}
              </button>
            </div>
